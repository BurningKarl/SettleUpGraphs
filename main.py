...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-b', action='store', dest='backend',
        default='bokeh', choices=['bokeh', 'pgf'],
        help='The backend: bokeh (html output) or pgf (pdf output)'
    )
    parser.add_argument(
        '-e', '--emojis', action='store_false', dest='translate_emojis',
        help='Disables translation from emojis to text'
    )
    parser.add_argument(
        '-l', action='store', metavar='LANG', dest='language',
        default=None,
        help='The language (e.g. en, de), fallback is English'
    )
    parser.add_argument(
        '-i', action='store', metavar='INPUT_FILE', dest='input_file',
        default='transactions.csv',
        help='The input file (default: "transactions.csv")'
    )
    parser.add_argument(
        '-o', action='store', metavar='OUTPUT_FILE', dest='output_file',
        default=None,
